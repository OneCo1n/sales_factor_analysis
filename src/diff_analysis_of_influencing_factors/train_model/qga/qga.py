...
class QGA(object):
    ...
    def __init__(self, population_size, chromosome_num, chromosome_length, max_value, min_value, iter_num, deta, X_train, y_train, X_test, y_test):
        '''初始化类参数
    ...
    def fitness(self, population_Binary):
        ...
        for i in range(len(population_Binary)):
            ...
            for j in range(len(population_Binary[i])):
                ...
                value = (total * (self.max_value - self.min_value)) / math.pow(2, len(
                    population_Binary[i][j])) + self.min_value  ##将十进制数值坐落在[min_value,max_value]之间
                tmp1.append(value)
        ...
        # 创建成lgb特征的数据集格式
        lgb_train = lgb.Dataset(self.X_train, self.y_train)  # 将数据保存到LightGBM二进制文件将使加载更快
        lgb_eval = lgb.Dataset(self.X_test, self.y_test, reference=lgb_train)  # 创建验证数据
        params = {
            'task': 'train',
        ...
        for l in range(len(parameters[0])):
            # model = lgb.train(params, lgb_train, valid_sets=lgb_eval)  # 训练数据需要参数列表和数据集
            model = lgb.train(params, lgb_train, num_boost_round=30, valid_sets=lgb_eval,
                      early_stopping_rounds=100)  # 训练数据需要参数列表和数据集
            yhat = model.predict(self.X_test, num_iteration=model.best_iteration)
            error = rmspe(np.expm1(yhat), np.expm1(self.y_test))
            fitness_value.append(error)
    ...
    def mutation(self, population_Angle_crossover, population_Angle, best_parameter_Binary, best_fitness):
        ...
        parameters, fitness_crossover, best_parameter_Binary_crossover, best_fitness_crossover, best_parameter = self.fitness(
            population_Binary_crossover)  ## 交叉后的适应度函数值列表
        ##2.初始化每一个量子位的旋转角度
        Rotation_Angle = []
    ...
    def main(self):
        ...
        for i in range(self.iter_num):
            ...
            parameters, fitness_value, current_parameter_Binary, current_fitness, current_parameter = self.fitness(
                population_Binary)
...
# # print('----------------1.Load Data-------------------')
# # trainX, trainY = load_data('F:\\论文\\code\\QGA_python-master\\QGA_python-master\\rbf_data')
...
# deta = 0.1 * np.pi
# print('----------------3.QGA-----------------')
# qga = QGA(population_size, chromosome_num, chromosome_length, max_value, min_value, iter_num, deta, X_train, y_train, X_test, y_test)
# qga.main()
