...
## 1.加载数据
from flaskTest.views.data_views.total_views import get_total_by_material_and_company
...
## 2. PSO优化算法
class PSO(object):
    def __init__(self, particle_num, particle_dim, iter_num, c1, c2, w, max_value, min_value, X_train, X_test, y_train, y_test):
        '''参数初始化
    ...
    def fitness(self, particle_loc, X_train, X_test, y_train, y_test):
        '''计算适应度函数值
        input:self(object):PSO类
        ...
        for i in range(self.particle_num):
            rbf_svm = svm.SVC(kernel='rbf', C=particle_loc[i][0], gamma=particle_loc[i][1])
            cv_scores = model_selection.cross_val_score(rbf_svm, X_train, y_train, cv=3, scoring='accuracy')
            fitness_value.append(cv_scores.mean())
    ...
    def main(self):
        ...
        for i in range(self.iter_num):
            ### 3.1 计算当前适应度函数值列表
            current_fitness_value, current_best_fitness, current_best_parameter = self.fitness(particle_loc,X_train, X_test, y_train, y_test)
...
min_value = 0.001
print('----------------3.PSO-----------------')
pso = PSO(particle_num, particle_dim, iter_num, c1, c2, w, max_value, min_value, X_train, X_test, y_train, y_test)
pso.main()
