...
from flaskTest.views.data_views.total_views import get_total_by_material, get_total_desc_by_material, get_total_people, \
    ...
    get_total_people_by_keyanliang, get_total_people_desc_by_keyanliang
...
# 首先获取所有相关数据
def do_analysis_number_of_people_plant_keyanliang(start_time, end_time, keyanliang):

    # 油站信息、poi信息，用于模型训练（已编码）
    data = get_total_people_by_keyanliang(start_time, end_time, keyanliang)
    # 更改列名
    ...
    data_desc = get_total_people_desc_by_keyanliang(start_time, end_time, keyanliang)
    # 更改列名
...
do_analysis_number_of_people_plant_keyanliang('2019-09-01', '2019-12-01', '')