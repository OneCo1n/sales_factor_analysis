...
from flaskTest.views.data_views.total_views import get_total_by_material, get_total_desc_by_material, \
    ...
    get_total_desc_by_category_and_keyanliang, get_total_by_category_and_stars, get_total_desc_by_category_and_stars
...
def do_analysis_category_material_plant_group_by_stars(category, start_time, end_time, stars):

    # 获取该商品的销量数据、油站信息、poi信息、促销信息等，用于模型训练（已编码）
    data = get_total_by_category_and_stars(category, start_time, end_time, stars)
    ...
    data_desc = get_total_desc_by_category_and_stars(category, start_time, end_time, stars)
    # 更改列名
...
do_analysis_category_material_plant_group_by_stars('包装饮料', '2019-09-01', '2019-12-01', '五星级')